...
describe("Rooms API (App Router handler)", () => {
  ...
  beforeAll(async () => {
    ...
    testPrefix = `room-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  it("PATCH /api/rooms/[id] actualiza solo los campos enviados", async () => {
    const body = { pricePerNight: 120 };
    const req = createMockRequest(`http://localhost:3000/api/rooms/${roomId}`, body);
    const res = await PATCH(req, { params: { id: String(roomId) } });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.success).toBe(true);
    expect(json.data.pricePerNight).toBe(120);
    // Verificar que otros campos no se hayan modificado
    expect(json.data.type).toBe("SENCILLA");
    expect(json.data.capacity).toBe(2);
  });

  it("PATCH /api/rooms/[id] actualiza el tipo de habitación", async () => {
    const body = { type: "DOBLE", capacity: 4 };
    const req = createMockRequest(`http://localhost:3000/api/rooms/${roomId}`, body);
    const res = await PATCH(req, { params: { id: String(roomId) } });
    expect(res.status).toBe(200);
    const json = await res.json();
  ...
  it("PATCH /api/rooms/[id] actualiza amenidades", async () => {
    const body = { amenities: ["WiFi", "TV", "AC", "Minibar"] };
    const req = createMockRequest(`http://localhost:3000/api/rooms/${roomId}`, body);
    const res = await PATCH(req, { params: { id: String(roomId) } });
    expect(res.status).toBe(200);
    const json = await res.json();
    expect(json.success).toBe(true);
    expect(json.data.amenities).toEqual(["WiFi", "TV", "AC", "Minibar"]);
  });
});
 ...
 * - Se cubren casos exitosos, ID inválido, habitación inexistente y actualizaciones parciales.
